                    ...
                    <div class="product-description">
                      ...
                      <form id="geniusformdata" action="{{route('admin-city-update',$data->id)}}" method="POST">
                          ...
                          <div class="col-lg-7">
                              <select name="category_id" required="">
                                  <option value="">{{ __("Select Country") }}</option>
                                    @foreach($cats as $cat)
                                      <option value="{{ $cat->id }}" {{ $data->country_id == $cat->id ? 'selected' :'' }}>{{ $cat->country_name }}</option>
                                    @endforeach
                                </select>
                          </div>
                        </div>
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-7">
                            <input type="text" class="input-field" name="city_name" placeholder="{{ __("Enter Name") }}" required="" value="{{$data->city_name}}">
                          </div>
                        </div>
